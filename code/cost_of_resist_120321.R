...
resc <- read.csv("data/cost_res_o.csv", header = T)

resc <- filter(resc, ! phage == 'y')

#resc <- filter(resc, ! res == "a")
...
mod1 <- lmer(m ~ res * treat + (1|gen), data = resc)
mod2 <- lmer(m ~ res + treat + (1|gen), data = resc)
anova(mod1,mod2)
